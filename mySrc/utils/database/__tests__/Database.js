...
describe('測試 Database', () => {
  ...
  test('getAll', async () => {
    ...
    await expect(database.getAll(STORE_NAME)).resolves.toBe(JSON.stringify([ans1, ans2]));
  });
  ...
  test('clear', async () => {
    ...
    await database.set(STORE_NAME, ans);
    await database.clear(STORE_NAME);
    await expect(database.getAll(STORE_NAME)).resolves.toBe('[]');
  });
  ...
  test('delete', async () => {
    ...
    await expect(database.getAll(STORE_NAME)).resolves.toBe(JSON.stringify([ans2]));
  });
